...
import { DeleteUser } from "@/services/user.servies/user.services";
import {
  Dialog,
  DialogPanel,
  DialogTitle,
...
export default function ModalDeleteUser({ onRefresh }: Props) {
  // state redux
  const dispatch = useAppDispatch();
  const isOpen = useSelector(
    (state: RootState) => state.modal.modalUser.isDeleteUser
  );
  ...
  // handler
  const CloseModal = () => {
    dispatch(setShowModalUser.userDelete(false));
  };

  const handleSubmit = async () => {
    // Not allow delete own account
    if (inforUser?.id === accountUser?.id) {
    ...
    if (inforUser?.id) {
      try {
        const res = await DeleteUser(inforUser.id);
              ...
              >
                ...
                <div className="mt-2">
                  <p className="text-m text-gray-500">
                    Bạn có chắn muốn xóa người dùng {inforUser?.email}
                  </p>
                </div>
