...
export default function ModalDeleteProject({ onRefresh }: Props) {
  ...
  const isOpen = useSelector(
    (state: RootState) => state.modal.modalUser.isDeleteProjectFromGroup
  );
  ...
  // handler
  const CloseModal = () => {
    dispatch(setDetailGroupId.deleteProject(false));
  };
    ...
    if (inforProject?.id) {
      try {
        ...
        if (res.statusCode === 200) {
          toast.success("Xóa dự án khỏi nhóm thành công");
          onRefresh();
          CloseModal();
        } else {
