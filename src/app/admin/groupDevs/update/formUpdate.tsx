...
type Props = {
  setDataUpdateGroupDev: React.Dispatch<
...
const FormCreateGroup = ({ dataUpdateGroup, setDataUpdateGroupDev }: Props) => {
  //selection
  ...
  const [selectedOption, setSelectedOption] = useState<SingleValue<OptionType>>(
    options.find((opt) => opt.value === dataUpdateGroup.visibility) || null
  );

  // handler
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setDataUpdateGroupDev((prev) => ({
      ...prev,
      [name]: value,
    }));
  };
  ...
  return (
    <form className="max-w-xl w-full mx-auto p-8 rounded-2xl shadow-lg bg-white">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-4">
        ...
        <div className="w-full md:w-1/2">
          ...
          <Input
            ...
            value={dataUpdateGroup.maxMembers}
            onChange={handleInputChange}
          />
        </div>
      </div>
      ...
      <label className="text-gray-500 text-sm">Mô tả</label>
      <textarea
        name="description"
        ...
        placeholder="Mô tả..."
        value={dataUpdateGroup.description}
        onChange={(e) =>
          setDataUpdateGroupDev((prev) => ({
            ...prev,
